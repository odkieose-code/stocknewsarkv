            ...
            for item in items:
                try:
                    ...
                    try:
                        ...
                        if analysis:
                            if analysis and not analysis.is_finance_related:
...
async def get_beneficiary_stocks(limit: int = 10):
    from sqlalchemy import select
    from datetime import datetime, timedelta
    cutoff = datetime.utcnow() - timedelta(hours=24)
    async with AsyncSessionLocal() as db:
        result = await db.execute(
            select(News.beneficiary_stocks)
            .where(News.crawled_at >= cutoff)
            .where(News.is_duplicate == False)
            .where(News.beneficiary_stocks != None)
        )
    ...
    for stocks in rows:
        ...
        for s in stocks:
            ...
            if name not in agg:
                agg[name] = {"name": name, "code": s.get("code"), "mention_count": 0, "impacts": [], "reasons": []}
            agg[name]["mention_count"] += 1
            if s.get("impact"): agg[name]["impacts"].append(s["impact"])
            if s.get("reason"): agg[name]["reasons"].append(s["reason"])
    score_map = {"상": 3, "중": 2, "하": 1}
    reverse_map = {3: "상", 2: "중", 1: "하"}
    output = []
    for item in agg.values():
        scores = [score_map.get(i, 2) for i in item["impacts"]]
        avg = round(sum(scores) / len(scores)) if scores else 2
        output.append({"name": item["name"], "code": item["code"],
            "mention_count": item["mention_count"],
            "avg_impact": reverse_map.get(avg, "중"),
