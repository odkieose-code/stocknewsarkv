...
function SystemStatus() {
  ...
  return (
    <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-tertiary)', lineHeight: 1.8, letterSpacing: '0.04em' }}>
      ...
      <div style={{ marginTop: 8, color: 'var(--text-ghost)' }}>SYS_TIME: {time.toLocaleTimeString('ko-KR', { hour12: false })}</div>
    </div>
  )
}
...
function MarketPulse({ newsData }: { newsData?: { items: NewsItem[]; total: number } }) {
  ...
  return (
    <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', fontFamily: 'var(--font-mono)', background: 'var(--bg-card)' }}>
      <div style={{ padding: '11px 14px', borderBottom: '1px solid var(--border)', fontSize: 10, color: 'var(--text-ghost)', letterSpacing: '0.1em' }}>// MARKET PULSE</div>
      <div style={{ padding: 14 }}>
        ...
        {hot.length > 0 ? (
                ...
                <a key={n.id} href={n.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  ...
                  >
                    <div style={{ display: 'flex', gap: 6, marginBottom: 3 }}>
                      <span style={{ fontSize: 9, color: 'var(--text-ghost)', flexShrink: 0, marginTop: 1 }}>0{idx + 1}</span>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', paddingLeft: 18 }}>
                      ...
                      <span style={{ fontSize: 8, color: n.sentiment === 'positive' ? 'var(--positive)' : n.sentiment === 'negative' ? 'var(--negative)' : 'var(--text-ghost)', letterSpacing: '0.06em' }}>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-ghost)', textAlign: 'center', padding: '10px 0', borderTop: '1px solid var(--border)', marginTop: 4 }}>
            <div>데이터 로딩 중...</div>
            <div style={{ fontSize: 9, opacity: 0.5, marginTop: 2 }}>FETCHING MARKET DATA</div>
          </div>
        ) : null}
      </div>
    </div>
...
function getNewsLabels(news: NewsItem) {
  ...
  if (news.sentiment === 'positive') labels.push({ text: '▲ 긍정', color: 'rgba(0,200,100,0.9)', bg: 'rgba(0,200,100,0.08)' })
  else if (news.sentiment === 'negative') labels.push({ text: '▼ 부정', color: 'rgba(255,70,70,0.9)', bg: 'rgba(255,70,70,0.08)' })
  if (news.importance_score) {
    if (news.importance_score >= 90) labels.push({ text: '🔥 HOT', color: 'rgba(255,140,0,0.9)', bg: 'rgba(255,140,0,0.08)' })
    else if (news.importance_score >= 80) labels.push({ text: '↑ 주목', color: 'rgba(80,140,255,0.9)', bg: 'rgba(80,140,255,0.08)' })
  }
  if (/급등|폭등|상한가|신고가/.test(title)) labels.push({ text: '급등', color: 'rgba(0,200,100,0.9)', bg: 'rgba(0,200,100,0.08)' })
  if (/급락|폭락|하한가|신저가/.test(title)) labels.push({ text: '급락', color: 'rgba(255,70,70,0.9)', bg: 'rgba(255,70,70,0.08)' })
  if (/실적|영업이익|매출|흑자|적자/.test(title)) labels.push({ text: '실적', color: 'rgba(160,100,255,0.9)', bg: 'rgba(160,100,255,0.08)' })
  if (/공시|공모|상장|IPO/.test(title)) labels.push({ text: '공시', color: 'rgba(255,100,100,0.9)', bg: 'rgba(255,100,100,0.08)' })
  if (/AI|반도체|배터리|2차전지/.test(title)) labels.push({ text: '테마', color: 'rgba(0,200,180,0.9)', bg: 'rgba(0,200,180,0.08)' })
  if (/미국|연준|Fed|금리|환율/.test(title)) labels.push({ text: '매크로', color: 'rgba(255,200,0,0.9)', bg: 'rgba(255,200,0,0.08)' })
  return labels.slice(0, 3)
}

// ── 인라인 뉴스카드 ───────────────────────────────────────
function NewsCardInline({ news, onClick, selected, isMobile }: { news: NewsItem; onClick: () => void; selected: boolean; isMobile: boolean }) {
  const labels = getNewsLabels(news)
  const sentiColor = news.sentiment === 'positive' ? 'var(--positive)' : news.sentiment === 'negative' ? 'var(--negative)' : 'var(--text-ghost)'
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 6,
        background: selected ? 'var(--accent-soft)' : 'rgba(255,255,255,0.025)',
        cursor: 'pointer',
        transition: 'border-color 0.15s, background 0.15s',
        fontFamily: 'var(--font-mono)',
      }}
      onMouseEnter={e => { if (!selected) { (e.currentTarget as HTMLDivElement).style.background = 'rgba(255,255,255,0.045)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--border-strong)' } }}
      onMouseLeave={e => { if (!selected) { (e.currentTarget as HTMLDivElement).style.background = 'rgba(255,255,255,0.025)'; (e.currentTarget as HTMLDivElement).style.borderColor = 'var(--border)' } }}
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 6 }}>
        {(news.importance_score ?? 0) >= 80 && (
          <span style={{ fontSize: 9, color: 'var(--accent)', letterSpacing: '0.1em', background: 'var(--accent-soft)', padding: '2px 6px', borderRadius: 3, whiteSpace: 'nowrap', marginTop: 2, flexShrink: 0 }}>HOT</span>
        )}
        <p style={{ fontSize: 12, color: 'var(--text-primary)', lineHeight: 1.6, margin: 0, fontWeight: 500, flex: 1 }}>{news.title}</p>
      </div>
      {news.summary && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        ...
        {news.sector && (
          <span style={{ fontSize: 9, padding: '1px 6px', border: '1px solid var(--border)', borderRadius: 3, color: 'var(--text-ghost)' }}>{news.sector}</span>
        )}
        ...
        <span style={{ fontSize: 9, color: 'var(--text-ghost)', marginLeft: 'auto' }}>{news.source} · {timeAgo}</span>
      </div>
    </div>
  )
}
      ...
      {isMobile && (
        <>
          <div
            ...
            style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, opacity: selectedNews ? 1 : 0, pointerEvents: selectedNews ? 'auto' : 'none', transition: 'opacity 0.25s' }}
          />
          <div style={{
            ...
            maxHeight: '80vh',
            overflowY: 'auto',
            transform: selectedNews ? 'translateY(0)' : 'translateY(100%)',
              ...
              <>
                ...
                <div style={{ padding: '14px 16px 32px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                    ...
                    <button onClick={() => setSelectedNews(null)} style={{ fontSize: 18, color: 'var(--text-ghost)', background: 'none', border: 'none', cursor: 'pointer', lineHeight: 1 }}>×</button>
                  </div>
                  ...
                  <a href={toMobileUrl(selectedNews.url)} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'inline-block', fontSize: 11, color: 'var(--accent)', letterSpacing: '0.08em', textDecoration: 'none', padding: '8px 14px', border: '1px solid var(--accent)', borderRadius: 4 }}>
                    원문 보기 →
                  </a>
                </div>
              </>
        ...
        <div style={{ display: isMobile ? 'block' : 'grid', gridTemplateColumns: '1fr 290px', gap: 20 }}>
          ...
          <section>
            ...
            {isLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[1,2,3,4,5].map(i => (
                  <div key={i} style={{ height: 75, background: 'rgba(255,255,255,0.025)', border: '1px solid var(--border)', borderRadius: 6, animation: 'breathe 1.5s ease infinite', animationDelay: `${i * 0.1}s` }} />
                ))}
              </div>
            ) : isError ? (
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-ghost)', padding: '32px 24px', textAlign: 'center', border: '1px solid var(--border)', borderRadius: 6 }}>
                <div style={{ marginBottom: 6, color: 'var(--negative)' }}>FEED DISCONNECTED</div>
                <div style={{ fontSize: 10, opacity: 0.6 }}>백엔드 서버에 연결할 수 없습니다</div>
              </div>
            ) : (
          ...
          {/* PC 사이드바 */}
          {!isMobile && (
            <aside style={{ display: 'flex', flexDirection: 'column', gap: 14, position: 'sticky', top: 68, alignSelf: 'flex-start' }}>
              <MarketPulse newsData={newsData} />
              <StockPanel />
              ...
              {selectedNews && (
                <div style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: 'var(--bg-card)', fontFamily: 'var(--font-mono)' }}>
                  {selectedNews.thumbnail && (
                    <img src={selectedNews.thumbnail} alt="" style={{ width: '100%', height: 150, objectFit: 'cover', display: 'block' }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none' }} />
                  )}
                  <div style={{ padding: 13 }}>
                    ...
                    {selectedNews.summary && (
                      <p style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 10 }}>{selectedNews.summary}</p>
                    )}
                    <a href={selectedNews.url} target="_blank" rel="noopener noreferrer"
                      style={{ fontSize: 10, color: 'var(--accent)', letterSpacing: '0.08em', textDecoration: 'none' }}>
                      OPEN SOURCE →
                    </a>
                  </div>
                </div>
